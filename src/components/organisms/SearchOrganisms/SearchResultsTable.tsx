...
import useInfiniteGetSearchReq from "@/hooks/useInfiniteGetSearchReq";
...
function SearchResultsTable() {
  ...
  const { data, fetchNextPage } = useInfiniteGetSearchReq();
  ...
  if (data?.pages[0]?.status !== 204) {
    return (
      <div className="max-w-screen-2xl m-auto flex flex-col gap-4 lg:gap-10">
        <TableWrapper
          headerTitle={t("searchTableTitle")}
          headerLink={
            <Button
          ...
          tbody={
            <tbody>
              {data?.pages?.map((data: any, pagesIndex: number) => {
                return data?.data?.searchResult?.map(
